...
class ConnectionManager {
  ...
  static ConnectionManager getInstanceWithAddressObject(
  ...
  static ConnectionManager getInstance() =>
      getInstanceWithAddressObject(mArgument);
...
Future<PingArgument> pingForHosts(String domain, String port) async {
  ...
  try {
    ...
    String responseBody = utf8.decode(result.bodyBytes);
    debugPrint(responseBody);
    var isAlive = result.statusCode == 200;
    print("Ping $isAlive");
    return PingArgument(isAlive, domain, port, responseBody);
  } catch (_) {
    print("Ping Error");
...
/*
Future<Stream<PingArgument>>scanForHosts(String port) async{
  ...
  return Future.value(_discoverHost(ip, port));
...
Stream<PingArgument> discoverHost() {
  ...
  Wifi.ip.then((ip) {
    ...
    for (int i = 1; i <= 255; i++) {
      ...
      futures.add(f);
      f.then((argument) {
        debugPrint("Ping Future Argument" + argument.deviceName +
